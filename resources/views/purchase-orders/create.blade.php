...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Créez une commande fournisseur.</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('purchase-orders.store') }}" class="space-y-6" id="poForm">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="supplier_id" id="supplier_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500" required>
                        ...
                        @foreach($suppliers as $supplier)
                            <option value="{{ $supplier->id }}" {{ old('supplier_id', $selectedSupplier?->id) == $supplier->id ? 'selected' : '' }}>{{ $supplier->company_name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="order_date" id="order_date" value="{{ old('order_date', date('Y-m-d')) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500" required>
                ...
                <div>
                    ...
                    <input type="date" name="expected_date" id="expected_date" value="{{ old('expected_date') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                </div>
            </div>
        </div>
                ...
                <table class="min-w-full" id="itemsTable">
                    ...
                    <tbody id="itemsBody">
                        <tr class="item-row border-b border-gray-100">
                            <td class="py-3 pr-2">
                                <select name="items[0][product_id]" class="product-select w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500" required>
                                    <option value="">Sélectionner</option>
                            ...
                            <td class="py-3 px-2">
                                <input type="number" name="items[0][quantity]" class="quantity-input w-full px-3 py-2 border border-gray-300 rounded-lg text-sm text-right focus:ring-2 focus:ring-primary-500" value="1" min="0.01" step="0.01" required>
                            </td>
                            <td class="py-3 px-2">
                                <input type="number" name="items[0][unit_price]" class="price-input w-full px-3 py-2 border border-gray-300 rounded-lg text-sm text-right focus:ring-2 focus:ring-primary-500" value="0" min="0" step="0.01" required>
                            </td>
                            <td class="py-3 px-2">
                                <input type="number" name="items[0][tax_rate]" class="tax-input w-full px-3 py-2 border border-gray-300 rounded-lg text-sm text-right focus:ring-2 focus:ring-primary-500" value="20" min="0" max="100" step="0.01">
                            </td>
            ...
            <div class="mt-6 flex justify-end">
                <div class="w-64 space-y-2">
                    ...
                    <div class="flex justify-between text-sm">
                        ...
                        <span id="taxTotal" class="font-medium">0</span>
                    </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <textarea name="notes" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500" placeholder="Notes internes...">{{ old('notes') }}</textarea>
        </div>

        <div class="flex items-center justify-end gap-4">
            <a href="{{ route('purchase-orders.index') }}" class="px-4 py-2 text-gray-700 font-medium rounded-lg hover:bg-gray-100 transition-colors">Annuler</a>
            <button type="submit" class="px-6 py-2 bg-primary-600 text-white font-medium rounded-lg hover:bg-primary-700 transition-colors">Créer le bon de commande</button>
        </div>
    </form>
</div>
