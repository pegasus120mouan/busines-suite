...
<div class="max-w-4xl mx-auto">
    ...
    <form method="POST" action="{{ route('products.store') }}" class="space-y-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <label class="relative flex cursor-pointer rounded-lg border border-gray-300 bg-white p-4 shadow-sm hover:border-primary-500 has-[:checked]:border-primary-600 has-[:checked]:ring-2 has-[:checked]:ring-primary-600">
                    <input type="radio" name="type" value="product" class="sr-only" {{ old('type', 'product') === 'product' ? 'checked' : '' }}>
                ...
                <label class="relative flex cursor-pointer rounded-lg border border-gray-300 bg-white p-4 shadow-sm hover:border-primary-500 has-[:checked]:border-primary-600 has-[:checked]:ring-2 has-[:checked]:ring-primary-600">
                    <input type="radio" name="type" value="service" class="sr-only" {{ old('type') === 'service' ? 'checked' : '' }}>
                    <span class="flex flex-1">
                        <span class="flex flex-col">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('name') border-red-500 @enderror" required>
                    @error('name')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="sku" class="block text-sm font-medium text-gray-700 mb-1">SKU / Référence</label>
                    <input type="text" name="sku" id="sku" value="{{ old('sku') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                </div>
                <div>
                    <label for="barcode" class="block text-sm font-medium text-gray-700 mb-1">Code-barres</label>
                    <input type="text" name="barcode" id="barcode" value="{{ old('barcode') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                </div>
                <div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <div class="relative">
                        <input type="number" name="purchase_price" id="purchase_price" value="{{ old('purchase_price') }}" step="0.01" min="0" class="w-full px-4 py-2 pr-12 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" name="selling_price" id="selling_price" value="{{ old('selling_price') }}" step="0.01" min="0" class="w-full px-4 py-2 pr-12 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('selling_price') border-red-500 @enderror" required>
                        <span class="absolute right-3 top-2.5 text-gray-500">{{ $currencySymbol }}</span>
                    </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" name="tax_rate" id="tax_rate" value="{{ old('tax_rate', 20) }}" step="0.1" min="0" max="100" class="w-full px-4 py-2 pr-8 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                ...
                <div>
                    ...
                    <select name="unit" id="unit" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                        <option value="unit" {{ old('unit', 'unit') === 'unit' ? 'selected' : '' }}>Unité</option>
                        <option value="hour" {{ old('unit') === 'hour' ? 'selected' : '' }}>Heure</option>
                        <option value="day" {{ old('unit') === 'day' ? 'selected' : '' }}>Jour</option>
                        <option value="kg" {{ old('unit') === 'kg' ? 'selected' : '' }}>Kilogramme</option>
                        <option value="m" {{ old('unit') === 'm' ? 'selected' : '' }}>Mètre</option>
                        <option value="m2" {{ old('unit') === 'm2' ? 'selected' : '' }}>Mètre carré</option>
                        <option value="l" {{ old('unit') === 'l' ? 'selected' : '' }}>Litre</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <label class="flex items-center">
                    <input type="checkbox" name="track_stock" value="1" {{ old('track_stock', true) ? 'checked' : '' }} class="w-4 h-4 text-primary-600 border-gray-300 rounded focus:ring-primary-500">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="min_stock_level" id="min_stock_level" value="{{ old('min_stock_level', 0) }}" min="0" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                    <p class="mt-1 text-xs text-gray-500">Alerte quand le stock atteint ce niveau</p>
                </div>
                <div>
                    <label for="reorder_point" class="block text-sm font-medium text-gray-700 mb-1">Point de réapprovisionnement</label>
                    <input type="number" name="reorder_point" id="reorder_point" value="{{ old('reorder_point', 0) }}" min="0" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <p class="mt-1 text-xs text-gray-500 ml-6">Les produits inactifs ne peuvent pas être ajoutés aux devis et factures</p>
        </div>
        ...
        <div class="flex items-center justify-end gap-4">
            ...
            <button type="submit" class="px-6 py-2 bg-primary-600 text-white font-medium rounded-lg hover:bg-primary-700 transition-colors">Créer le produit</button>
        </div>
    </form>
</div>
@endsection
