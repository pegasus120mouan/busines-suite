...
<div class="max-w-5xl mx-auto">
    <div class="mb-6">
        <a href="{{ route('quotes.index') }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 mb-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <input type="date" name="quote_date" id="quote_date" value="{{ old('quote_date', date('Y-m-d')) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500" required>
                </div>
                <div>
                    <label for="valid_until" class="block text-sm font-medium text-gray-700 mb-1">Valide jusqu'au *</label>
                    <input type="date" name="valid_until" id="valid_until" value="{{ old('valid_until', date('Y-m-d', strtotime('+30 days'))) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500" required>
                </div>
                <div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <textarea name="terms" id="terms" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">{{ old('terms', 'Devis valable 30 jours. Acompte de 30% à la commande.') }}</textarea>
                </div>
            </div>
        </div>

        <div class="flex items-center justify-end gap-4">
            <a href="{{ route('quotes.index') }}" class="px-6 py-2 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50">Annuler</a>
            <button type="submit" class="px-6 py-2 bg-primary-600 text-white font-medium rounded-lg hover:bg-primary-700">Créer le devis</button>
        </div>
    </form>
</div>
    ...
    row.innerHTML = `
        <td class="py-3 pr-2">
            <select name="items[${itemIndex}][product_id]" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" onchange="selectProduct(this)">
                ...
                ${products.map(p => `<option value="${p.id}">${p.name} - ${formatPrice(p.selling_price)} ${currencySymbol}</option>`).join('')}
            </select>
            <input type="text" name="items[${itemIndex}][description]" placeholder="Description *" class="w-full mt-2 px-3 py-2 border border-gray-300 rounded-lg text-sm" required>
        </td>
        <td class="py-3 px-2">
        ...
        <td class="py-3 px-2">
            <input type="number" name="items[${itemIndex}][unit_price]" value="0" min="0" step="0.01" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm text-right item-price" onchange="calculateRow(this)" required>
        </td>
        <td class="py-3 px-2">
            <input type="number" name="items[${itemIndex}][tax_rate]" value="20" min="0" max="100" step="0.1" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm text-right item-tax" onchange="calculateRow(this)">
        </td>
