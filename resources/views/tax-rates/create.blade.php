...
@section('title', 'Nouveau Taux de TVA')
...
<div class="max-w-xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Nouveau Taux de TVA</h1>
            <p class="mt-1 text-sm text-gray-500">Ajoutez un taux de TVA.</p>
        </div>
    </div>

    <form method="POST" action="{{ route('tax-rates.store') }}" class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        @csrf

        <div>
            <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Nom *</label>
            <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="TVA normale, TVA réduite..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500" required>
            @error('name')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
        </div>

        <div>
            <label for="rate" class="block text-sm font-medium text-gray-700 mb-1">Taux (%) *</label>
            <input type="number" name="rate" id="rate" value="{{ old('rate', 20) }}" step="0.01" min="0" max="100" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500" required>
